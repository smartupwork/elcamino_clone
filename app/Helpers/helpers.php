...
if (!function_exists('theme')) {
     ...
     *
     * @return \App\Theme\Adapters\Theme|\Illuminate\Contracts\Foundation\Application|mixed
     */
    function theme()
    {
        return app(\App\Theme\Adapters\Theme::class);
    }
}
...
if (!function_exists('util')) {
     ...
     *
     * @return \App\Theme\Adapters\Util|\Illuminate\Contracts\Foundation\Application|mixed
     */
    function util()
    {
        return app(\App\Theme\Adapters\Util::class);
    }
}
...
if (!function_exists('bootstrap')) {
    ...
    function bootstrap()
    {
        return app(\App\Theme\Bootstraps\BootstrapDemo1::class);
    }
}
...
if (!function_exists('assetCustom')) {
    ...
    function assetCustom($path)
    {
        ...
        if (isRTL()) {
            return asset(theme()->getThemeAssetsPath().'/'.dirname($path).'/'.basename($path, '.css').'.rtl.css');
        }
        ...
        if (theme()->isDarkModeEnabled() && theme()->getCurrentMode() !== 'light') {
            ...
            if (file_exists(public_path(theme()->getThemeAssetsPath().'/'.$darkPath))) {
                return asset(theme()->getThemeAssetsPath().'/'.$darkPath);
            }
        }

        // Include default css file
        return asset(theme()->getThemeAssetsPath().'/'.$path);
    }
}
